...
> **Model V0.1 Candidate は探索的スクリーニングで現在優先する実験的候補モデルです。Model V0 はリリース済み・凍結済みで再現可能なベースラインです。** V0.1 の独立した外部検証は未実施であり、V0 を置き換えたり非推奨にしたりするものではありません。

## 予測フロー

![DJR-MCP Finder の予測フロー](../assets/readme/readme_workflow.svg)

両モデルとも最終判定は H1→H2→H3 の順ですが、計算経路は異なります。Model V0 は共有する一つの ESM-C 6B 埋め込みから全配列の H1 と H2 の生スコアを計算し、H2 の結果は H1 陽性の場合だけ実際のカスケード判定に用います。Model V0.1 は全配列で H1 を計算し、H1 陽性配列だけで H2 を計算します。H3 は両モデルとも H1/H2 の両方が陽性の配列だけで計算します。すべてのスコア、ゲート通過状態、最終ラベルは `predictions.tsv` に記録されます。これらのラベルはスクリーニング用であり、構造確認を意味するものではありません。
...
V0.1 は V0 の凍結済みイメージをベースとして使用するため、リポジトリを初めてクローンした場合は最初に V0 をビルドする必要があります。上記の空値は、ローカルで再構築すると必ず変化する過去の Docker イメージ ID の確認だけを省略します。バージョン、環境、チェックサムの確認は引き続き実行されます。予測処理の本体は Python で実装されており、Docker は H1/H2 と H3 が必要とする二つの固定実行環境を分離するために使用されます。

通常の FASTA 予測に PBS、`qsub`、HPC ジョブスケジューラは不要です。

初回の予測時には、固定バージョンのモデル重みがダウンロードされます。結果は次の場所に保存されます。
...
四つのエビデンスグループには、完全一致配列を重複除去した代表タンパク質が合計 11,060 件含まれます。固定分割は Train 6,634 件、Validation 2,212 件、Test 2,214 件です。Validation/Test は開発データ内の固定パーティションです。以下のモデル選択では Train のみを使用しており、新たな独立外部テストには該当しません。
...
Model V0 は、一つの ESM-C 6B 埋め込みから全配列の H1 と H2 を計算し、H1/H2 の両方が陽性の配列だけで H3 を計算します。Model V0.1 Candidate は混合エンコーダーのカスケードです。ESM-2 3B と、それ専用に凍結された H1/H2 出力ヘッド、温度パラメータ、閾値が DJR/MCP スクリーニングを担当します。二段階の判定を通過した配列に対してのみ ESM-C 6B を実行し、V0 と完全に同一の H3 ウイルス門分類・拒否判定ヘッドを使用します。
...
| --- | --- | --- |
| プロジェクト上の位置づけ | リリース済み・凍結済みで再現可能な正式ベースライン | 探索的スクリーニングで優先する実験的候補。独立した外部検証は未実施 |
| H1/H2 | ESM-C 6B の埋め込み、出力ヘッド、校正 | ESM-2 3B の埋め込み、および対応する新しい凍結済み出力ヘッドと校正 |
| H3 | ESM-C 6B のウイルス門分類・拒否判定ヘッド | V0 とバイト単位で同一の H3 モデルファイルと校正 |
| 実行方式 | 全配列で H1/H2 生スコアを計算し、H2 は実際のカスケードで制御。H3 は H1/H2 両陽性の場合のみ計算 | 全配列で H1、H1 陽性配列だけで H2 を計算。第二エンコーダーと H3 は H1/H2 両陽性の場合のみ実行 |
| 出典・バージョン情報 | `predictions.tsv` は 20 フィールド | 23 フィールド。三つの出力ヘッド用エンコーダーフィールドを追加 |

図中のゲート値は、各バージョンで個別に凍結された校正閾値であり、性能スコアではありません。閾値の大小だけから、どちらのモデルが強い、または厳格であるかを判断することはできません。両バージョンは同一の三段階の決定規則と五種類の最終ラベルを維持していますが、V0.1 では実行経路と出典・バージョン情報がより明確になっています。

#### 平均性能

![Model V0 と Model V0.1 Candidate の Train-only 開発 Benchmark](../assets/readme/readme_train_cv_performance.svg)

図は五折平均 ± SE を示し、横軸の表示範囲は 0.968–1.000 です。V0.1 で置き換えられたのは H1/H2 系全体です。H2 AP は偶然同一であり、H3 は同じモデルファイルを再利用するため完全に同一です。
...
`S = 0.60 × H1 AP + 0.30 × H2 AP + 0.10 × H3 macro-F1` です。V0.1 の H1 AP は平均 `0.000833` 向上しました。H2/H3 は変化しないため、総合スコアの平均差 `+0.000500` は正確に `0.60 × ΔH1` に由来します。太字は候補モデルの選定を示すものであり、統計的有意性を示すものではありません。
...
- V0.1 の優先的な実験候補という位置づけは Train のみを用いた開発時 CV に基づき、独立した外部検証はまだ行われていません。
- スコアは自然試料での出現頻度を補正した確率ではありません。大規模スクリーニングには、独立した偽陽性評価と、構造または人手による確認が必要です。
...
| [`user-inference-v0.1/`](../../user-inference-v0.1/) | 探索的スクリーニングで優先する Model V0.1 Candidate 推論パッケージ |
...
研究で DJR-MCP Finder を使用する場合は、[`CITATION.cff`](../../CITATION.cff) に従って本ソフトウェアを引用し、使用したモデルが Model V0 か Model V0.1 Candidate かを明記してください。

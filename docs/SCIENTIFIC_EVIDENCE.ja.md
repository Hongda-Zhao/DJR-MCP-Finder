...
| --- | --- | --- | --- |
| **Model V0.1 Candidate** | H1/H2 に ESM-2 3B、H3 に ESM-C 6B | 外部確認を推奨 | **新規スクリーニングに推奨する現在の結果** |
| **Model V0** | H1/H2/H3 に ESM-C 6B | 公開済み・凍結済み | 正式な再現可能ベースラインおよびサポート対象のフォールバック |

「推奨」は、現在のスクリーニング経路と Train-CV の結果を表します。Model V0.1
...
| V0/V0.1 低類似性監査 | 内部確認に合格；正式な主張は保留 | 内部ホールドアウトと低カバレッジストレス層における記述的挙動 | 正式な `<20% identity` 結論 |
...
Model V0.1 Candidate が回収するウイルス strict clusters は 52/69 で、Model V0 の 55/69 より
少なく、H3 に到達する配列には二つ目のエンコーダーが必要です。引き続き外部確認を推奨し、
公開済み Model V0 を置き換えるものではありません。
